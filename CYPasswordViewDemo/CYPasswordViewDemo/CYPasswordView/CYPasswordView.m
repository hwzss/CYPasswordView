...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    if (!_textStr) {
        ...
        _textStr = [NSString stringWithFormat:@"%@%@",_textStr,string];
    }
    
    if ([string isEqualToString:@""]) {
        [CYNotificationCenter postNotificationName:CYPasswordViewDeleteButtonClickNotification object:self];
        if (_textStr.length > 0) {   //  删除最后一个字符串
            NSString *lastStr = [_textStr substringToIndex:[_textStr length] - 1];
            _textStr = lastStr;
        }
    } else {
        if (_textStr.length == kPWDLength) {
            if (self.finish) {
                self.finish(_textStr);
            }
            _textStr = nil;
        }
