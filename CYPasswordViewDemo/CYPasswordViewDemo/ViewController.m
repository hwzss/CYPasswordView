...
//  WZ_PwdVerify *model = [WZ_PwdVerify WZ_showPwdVerifyInView:self.view.window InputComplete:^(NSString *pwd, WZ_PwdVerify *verifyManger) {
//        [verifyManger WZ_LoadingStartWithMsg:@"申请中..."];
//        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kRequestTime * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                [verifyManger WZ_LoadingComplete:YES Msg:@"提现成功"];
//                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kDelay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
//                    [verifyManger WZ_hide];
//                });
//        });
//         
//    }];
//    
//    [model WZ_forgetPwdHandler:^{
//         NSLog(@"%s",__func__);
//    }];
//    [model WZ_cancelTapHandler:^{
//         NSLog(@"%s",__func__);
//    }];
...
//                });
                ...
                [weakSelf.passwordView stopLoading];
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(kDelay * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                    [weakSelf.passwordView inputAgain];
                });
